...
// tests that don't care about platform-aware sandbox configuration. It uses the
// Linux platform default, which is danger-full-access (#6218) — the per-task
// HOME redirect that used to pair with workspace-write on Linux has been
// retired, so the managed config no longer carries a sandbox_workspace_write
// block on this path.
