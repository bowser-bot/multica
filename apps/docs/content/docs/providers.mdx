...
## Codex sandbox and host access

<Callout type="warning">
On Linux and macOS, and on Windows unless a native Codex sandbox is configured, Multica starts Codex with `danger-full-access`. Commands can read and write any files available to the daemon user and can access the network. Run the daemon in a VM, container, dedicated OS account, or equivalent outer boundary when tasks need containment.
</Callout>

Multica preserves the daemon user's normal `HOME` and XDG environment so locally configured CLIs keep finding their credentials, configuration, sockets, and state. The per-task `CODEX_HOME` remains separate: Multica still uses it for managed Codex configuration, sessions, MCP, and skill injection. Sandbox selection does not rewrite the user's `approval_policy`.
