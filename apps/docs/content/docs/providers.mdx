...
## Access to the runtime computer

AI coding tools run on the computer that hosts the runtime. Permission controls vary by tool, but each process starts under the same operating-system account as the Multica daemon and may be able to use that account's files, developer tools, and credentials.

<Callout type="warning">
Treat a runtime as trusted access to its computer. If tasks should not share the daemon user's access, run Multica under a dedicated account or inside a VM or container.
</Callout>

Codex currently uses full-access mode on Linux and macOS, and on Windows unless its native sandbox is enabled. This allows tasks to use the developer tools already configured on the computer and to access the network.
