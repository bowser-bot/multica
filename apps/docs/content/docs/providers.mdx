...
## Host access and isolation

AI coding tools run on the runtime computer using the same operating-system account as the Multica daemon. Permission controls vary by tool.

On Linux and macOS, Codex tasks can read and write files available to that account and can use the network. The same applies on Windows unless Codex's native sandbox is enabled. Tools already set up for the account — such as git, GitHub CLI, and cloud CLIs — are available inside tasks with their existing settings and sign-ins.

<Callout type="warning">
To limit what Codex tasks can reach, run the Multica daemon under a dedicated OS account or place the runtime in a VM or container.
</Callout>
